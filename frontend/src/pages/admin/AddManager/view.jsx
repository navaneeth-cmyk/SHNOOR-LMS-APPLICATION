    ...
    if (loading) return (
        <div className="flex items-center justify-center min-h-[500px]">
            <div className="flex flex-col items-center gap-4">
                ...
                <p className="text-slate-400 font-medium text-sm">Adding manager...</p>
            </div>
        </div>
    );
            ...
            <div className="relative overflow-hidden rounded-2xl p-6 lg:p-8" style={{ background: 'linear-gradient(135deg, #0f172a 0%, #1e293b 50%, #312e81 100%)' }}>
                <div className="relative z-10 flex items-center gap-4">
                    ...
                    <div className="w-12 h-12 rounded-xl bg-white/10 border border-white/20 flex items-center justify-center">
                        <UserPlus size={24} className="text-sky-300" />
                    </div>
                    <div>
                        <h1 className="text-xl lg:text-2xl font-bold text-white tracking-tight">Add Manager</h1>
                        <p className="text-slate-400 text-sm mt-0.5">Fill in the details to register a new manager.</p>
                    </div>
                </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                ...
                <form onSubmit={handleSubmit} className="p-6 lg:p-8">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        ...
                        <div className="space-y-2">
                            <label className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">College Name</label>
                            <div className="relative group">
                                ...
                                <input name="college" value={data.college} onChange={handleChange} required
            ...
            {showSuccessPopup && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-8 text-center">
                        ...
                        <h3 className="text-xl font-bold text-primary-900 mb-2">Manager Added!</h3>
                        <p className="text-slate-400 text-sm mb-6">The manager account has been created and invite mail has been sent.</p>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col">
                        <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                            <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wider">Bulk Upload Managers</h3>
                                    ...
                                    <div className="p-4 bg-indigo-50 border border-indigo-100 rounded-xl space-y-2">
                                        <div className="flex gap-3">
                                            ...
                                            <p className="text-xs text-indigo-900 leading-relaxed">
                                                ...
                                                <span className="block font-bold mt-1">fullName, email, college, phone (opt), bio (opt)</span>
                                            </p>
                                        </div>
                                    </div>
                                ...
                                <div className="space-y-6">
                                    <div className="text-center space-y-2">
                                        ...
                                        <p className="text-slate-500 text-sm">
                                            Successfully added <span className="font-bold text-emerald-600">{bulkUploadResult.successCount}</span> managers.
                                        </p>
                                    </div>
                                                ...
                                                {bulkUploadResult.errors.map((err, idx) => (
                                                    <div key={idx} className="p-3">
                                                        <p className="text-xs text-amber-900 leading-relaxed font-medium">
                                                            {err.email ? `${err.email}: ` : err.row ? `Row ${err.row}: ` : ''}
                                                            <span className="font-normal">{err.message || err.error || 'Unknown error'}</span>
                                                        </p>
                                                    </div>
                                                ))}
                                            </div>
