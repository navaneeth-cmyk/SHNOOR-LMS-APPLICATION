...
const StudentChat = () => {
  ...
  const handleSendMessage = async (text, file, replyToId = null) => {
    ...
    const payload = {
      ...
      // FIX: dbUser.displayName and dbUser.fullName are both wrong field names;
      // the correct snake_case field used throughout this codebase is dbUser.full_name
      senderName: dbUser.full_name || "Student",
      attachment_file_id: attachmentFileId,
      attachment_name: attachmentName,
      attachment_type: attachmentType,
      reply_to_message_id: replyToId,
  ...
  // Single API call - backend already searches both DMs and groups
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-8 gap-8">
        ...
        <div className="flex flex-col gap-4 items-start lg:items-end w-full lg:w-auto">
          ...
          <div className="flex bg-white shadow-sm border border-slate-200 p-1.5 rounded-[20px] gap-1 w-full lg:w-auto">
            <button
              // FIX: was broken string concatenation in className — use proper template literal
